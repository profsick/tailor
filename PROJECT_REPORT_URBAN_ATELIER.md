# URBAN ATELIER

## CUSTOM TAILORING WEB APPLICATION

### Major Project Report

Submitted in partial fulfillment of the requirements for the award of the degree of

BACHELOR OF TECHNOLOGY / BACHELOR OF COMPUTER APPLICATIONS
(Replace with your actual degree)

Submitted by:

- Name: **\*\*\*\***\_\_\_\_**\*\*\*\***
- Roll No: **\*\*\*\***\_\_**\*\*\*\***
- Department: **\*\***\_\_\_\_**\*\***
- College: **\*\*\*\***\_\_\_\_**\*\*\*\***
- University: **\*\*\*\***\_**\*\*\*\***

Academic Year: 2025-2026

---

## CERTIFICATE

This is to certify that the project report entitled "Urban Atelier - Custom Tailoring Web Application" is a bonafide work carried out by **\*\*\*\***\_\_**\*\*\*\*** under my supervision and guidance in partial fulfillment of the requirements for the award of the degree **\*\*\*\***\_\_**\*\*\*\*** during the academic year 2025-2026.

Guide Signature: **\*\*\*\***\_\_**\*\*\*\***

Head of Department: **\*\*\*\***\_\_**\*\*\*\***

Date: **\*\*\*\***\_\_**\*\*\*\***

Place: **\*\*\*\***\_\_**\*\*\*\***

---

## DECLARATION

I hereby declare that this project report titled "Urban Atelier - Custom Tailoring Web Application" is my original work and has been carried out under the guidance of my project supervisor. This report has not been submitted to any other university or institution for the award of any degree, diploma, or certificate.

Student Signature: **\*\*\*\***\_\_**\*\*\*\***

Name: **\*\*\*\***\_\_**\*\*\*\***

Date: **\*\*\*\***\_\_**\*\*\*\***

---

## ACKNOWLEDGEMENT

I would like to express my sincere gratitude to my project guide for continuous support, valuable suggestions, and encouragement throughout the development of this project. I also thank the Head of Department, faculty members, and my friends for their guidance and help during the project work.

I am deeply thankful to my family for their constant motivation and moral support.

---

## PROJECT SUMMARY

Urban Atelier is a full-stack web application built to modernize custom tailoring operations by digitizing order placement, measurement capture, catalog management, and order tracking. The system provides a customer-facing storefront where users select clothing types, choose fabrics, save measurements, and place orders. It also provides an authenticated admin dashboard to monitor incoming orders, update order status, delete orders, and manage catalog entries.

The project integrates a static frontend built with HTML, CSS, and JavaScript with a backend API built using Node.js and Express, and it stores application data in MongoDB Atlas. The platform demonstrates practical use of REST APIs, browser storage, schema-driven data modeling, asynchronous communication, and role-based workflow separation between customers and administrators.

Key outcomes of the project include:

- Reduced manual order recording process.
- Better order visibility for both customer and admin.
- Centralized and scalable order persistence in MongoDB.
- Shared catalog updates across browsers and devices.
- Expandable architecture for future additions like payment and delivery.

---

## ABSTRACT

Traditional tailoring businesses often rely on manual notebooks, in-person measurement records, and phone-based updates. This method leads to communication delays, poor order visibility, and a higher chance of data loss. Urban Atelier addresses these problems through a web-based custom tailoring platform with both customer and administrator workflows.

The customer module allows users to browse tailoring options, select apparel categories, choose suitable fabrics, review cart data, and submit detailed orders along with body measurements and special instructions. The admin module includes a backend-validated login flow, a complete order table view, status update actions, deletion controls, and service-health checks.

On the technical side, the solution uses a decoupled architecture:

- Frontend pages manage user interactions and local state.
- A backend API validates and processes order and catalog operations.
- MongoDB stores finalized orders and catalog entries with structured fields.

The project emphasizes maintainability and extensibility. It uses modular route/controller/model separation in backend code and dedicated scripts for customer flow, admin dashboard operations, and catalog management. The admin login routes through the backend for credential verification and token-based session handling. Catalog images are stored in MongoDB as base64 strings so the clothing and fabric catalogue can be shared across browsers and devices.

The developed system successfully demonstrates an industry-relevant full-stack workflow and provides a practical foundation for real deployment in small and medium tailoring businesses.

Keywords: custom tailoring, full-stack web app, MongoDB, Express.js, order management, admin dashboard, JWT, catalog management.

---

## TABLE OF CONTENTS

| S. No. | TITLE                                     | PAGE NO. |
| ------ | ----------------------------------------- | -------: |
|        | PROJECT SUMMARY                           |        3 |
|        | ABSTRACT                                  |        4 |
| 1      | CHAPTER 1 - INTRODUCTION                  |        5 |
|        | 1.1 Introduction to the Project           |        6 |
|        | 1.2 System Specification                  |        7 |
|        | 1.2.1 Hardware Configuration              |        8 |
|        | 1.2.2 Software Specification              |        8 |
| 2      | CHAPTER 2 - SYSTEM STUDY                  |        9 |
|        | 2.1 Existing System                       |       10 |
|        | 2.2 Drawbacks of Existing System          |       10 |
|        | 2.3 Proposed System                       |       11 |
|        | 2.4 Advantages of Proposed System         |       11 |
| 3      | CHAPTER 3 - SYSTEM DESIGN AND DEVELOPMENT |       12 |
|        | 3.1 File Design                           |       13 |
|        | 3.2 Input Design                          |       14 |
|        | 3.3 Output Design                         |       14 |
|        | 3.4 Database Design                       |       15 |
|        | 3.5 System Development                    |       15 |
|        | 3.5.1 Description of Modules              |       16 |
| 4      | CHAPTER 4 - TESTING AND IMPLEMENTATION    |       17 |
|        | 4.1 System Testing                        |       18 |
|        | 4.2 Implementation                        |       19 |
| 5      | CHAPTER 5 - CONCLUSION                    |       20 |
|        | 5.1 Future Enhancements                   |       22 |
| 6      | BIBLIOGRAPHY                              |       23 |
| 7      | APPENDICES                                |       23 |
|        | DATA FLOW DIAGRAM (DFD)                   |       24 |
|        | DATABASE TABLE STRUCTURE                  |       24 |
|        | CODE SNIPPETS                             |       26 |
|        | SCREENSHOTS                               |       47 |

---

## PAGE PLANNING FOR 50+ PAGES

Format this report in Word or PDF using 12 pt font, 1.5 line spacing, and normal margins.
If you include the suggested screenshots, diagrams, and code snippets, the report comfortably reaches 50+ pages.

Recommended page distribution:

- Front matter: 8-10 pages
- Chapter 1: 6-8 pages
- Chapter 2: 7-8 pages
- Chapter 3: 14-18 pages
- Chapter 4: 7-8 pages
- Chapter 5: 3-4 pages
- Bibliography: 1-2 pages
- Appendices: 12-16 pages

Expected total: 58-66 pages.

---

# CHAPTER 1 - INTRODUCTION

## 1.1 Introduction to the Project

The tailoring industry is highly personalized, but many businesses still use fragmented manual systems for managing customer requirements, body measurements, and order tracking. In such environments, order history is difficult to retrieve, changes are hard to track, and communication with customers is inconsistent.

Urban Atelier is developed as a custom tailoring web platform that combines ordering, measurements, catalog browsing, and status updates into one unified solution. It targets three major goals:

1. Digitize the customer order journey.
2. Reduce manual overhead for tailor or admin staff.
3. Provide a transparent workflow where both customer and tailor can monitor order state.

The project includes:

- Customer storefront for product selection.
- Cart-based flow for multi-item custom orders.
- Measurement capture and review mechanism.
- Backend order persistence with MongoDB.
- Admin dashboard for order operations and catalog management.

This project demonstrates core software engineering principles such as modular architecture, input validation, error handling, state synchronization, and API-driven communication.

### Problem Statement

Manual tailoring operations face the following recurring issues:

- Orders are maintained in diaries or spreadsheets without structured validation.
- Customer measurements are often lost or overwritten.
- Order status updates are delayed and non-standardized.
- Admin has no real-time dashboard for pending or completed orders.

Hence, there is a need for a responsive and maintainable web application that formalizes order lifecycle management.

### Objectives

Primary objectives of Urban Atelier:

- Build a user-friendly interface for selecting clothing and fabric combinations.
- Enable customers to submit measurements and contact details with each order.
- Persist finalized orders and catalog entries into MongoDB.
- Provide admin controls to list, complete, and delete orders.
- Maintain a clear separation between customer UI logic and backend business logic.

### Scope of the Project

In-scope:

- Frontend pages for ordering and history tracking.
- Admin dashboard with order actions.
- REST API for order and catalog CRUD-related operations.
- MongoDB schema for order and catalog data.

Out-of-scope for the current phase:

- Online payment gateway integration.
- OTP-based authentication.
- Multi-admin role hierarchy.
- Real-time notifications through email or messaging apps.

### Significance

Urban Atelier provides a practical digital bridge for local tailoring businesses transitioning from offline workflow to online order processing. The architecture is intentionally lightweight for easy deployment and future expansion.

## 1.2 System Specification

System specification defines the minimum hardware and software environment required to build, run, and test the project reliably.

### 1.2.1 Hardware Configuration

Minimum requirements:

- Processor: Intel i3 or equivalent and above
- RAM: 4 GB minimum, 8 GB recommended
- Storage: 2 GB free disk space
- Internet: Required for MongoDB Atlas connectivity

Recommended requirements:

- Processor: Intel i5/i7 or equivalent
- RAM: 8 GB or more
- SSD storage for faster package installation and builds

### 1.2.2 Software Specification

Development tools and stack:

- OS: Windows 10/11
- Editor: Visual Studio Code
- Runtime: Node.js
- Package Manager: npm
- Backend Framework: Express.js
- Database: MongoDB Atlas
- ODM: Mongoose
- Version Control: Git
- Browser: Google Chrome or Microsoft Edge

Frontend technologies:

- HTML5
- CSS3
- Vanilla JavaScript (ES6+)

Backend dependencies:

- express
- mongoose
- cors
- dotenv
- bcryptjs
- jsonwebtoken
- nodemon for development

## 1.3 Importance of the System

This system matters because it removes several low-value manual tasks from the tailoring workflow. Instead of writing orders in notebooks and maintaining item catalogs by hand, the tailor can manage catalog data and order data through a structured web interface. Customers also receive a cleaner ordering experience, which reduces confusion and supports repeat usage.

---

# CHAPTER 2 - SYSTEM STUDY

## 2.1 Existing System

Traditional tailoring workflow is mostly manual:

- Customer visits the shop physically.
- Tailor notes measurements in a register.
- Garment choices and fabric notes are written as text.
- Progress updates happen through calls or direct follow-ups.
- Completion records are rarely structured.

In some semi-digital cases, shops use social media or messaging apps, but such tools are not designed for structured order records, validation, searchable history, or catalog management.

### Existing Workflow Characteristics

- Unstructured data entry
- No guaranteed backup
- No normalized order status model
- No easy retrieval by date or customer
- No customer self-service visibility

## 2.2 Drawbacks of Existing System

Operational drawbacks:

- Duplicate effort in collecting and rewriting customer details.
- Human error in recording measurements.
- Difficulty in handling multiple pending orders.
- No quick way to identify delayed orders.

Technical drawbacks:

- No central database.
- No API-based integration possibility.
- No modular software architecture.

Customer experience drawbacks:

- Uncertainty in order status.
- No digital trail for previous orders.
- Manual re-entry of details for repeat orders.

Business drawbacks:

- Limited scalability as customer volume grows.
- High dependency on specific staff members.
- Weak reporting and analytics capability.

## 2.3 Proposed System

Urban Atelier proposes a full-stack web system with two coordinated tracks:

Customer track:

1. Select clothing type.
2. Select fabric.
3. Add item to cart.
4. Provide measurements.
5. Submit order to backend.
6. Track order status on the orders page.

Admin track:

1. Log in to the admin dashboard.
2. View all orders from the backend.
3. Mark orders as completed.
4. Delete one or all orders when needed.
5. Manage catalog items from the add-items page.

### Proposed Architecture

Three-layer architecture is used:

- Presentation Layer: HTML/CSS/JS pages.
- Application Layer: Express routes and controllers.
- Data Layer: MongoDB with Mongoose schemas.

This ensures clean responsibility boundaries and easier maintenance.

## 2.4 Advantages of Proposed System

- Structured order capture with validation.
- Persistent storage in cloud database.
- Faster order lookup for admin.
- Better customer confidence through status visibility.
- Reusable and modular codebase.
- Easier to extend with payments, notifications, and analytics.
- Shared catalog updates across browsers and devices.

### Comparative Summary

| Feature              | Existing Manual System | Proposed Urban Atelier System    |
| -------------------- | ---------------------- | -------------------------------- |
| Order storage        | Notebook or ad-hoc     | MongoDB persistent storage       |
| Status tracking      | Phone-based            | API-driven status field          |
| Admin visibility     | Limited                | Dashboard with full order list   |
| Measurement handling | Paper entries          | Structured form and JSON storage |
| Scalability          | Low                    | Moderate to high                 |

---

# CHAPTER 3 - SYSTEM DESIGN AND DEVELOPMENT

## 3.1 File Design

The project is split into frontend files at the root and backend files under a Node app folder. This design keeps UI assets and API implementation independent while still allowing a simple deployment setup.

### 3.1.1 Frontend File Organization

Core customer pages:

- index.html: landing page and product/fabric selection.
- cart.html: cart listing, quantity updates, measurement review.
- order.html: final order placement form.
- yorders.html: customer order history and status tracking.

Core admin pages:

- admin.html: login gate and dashboard shell.
- add-items.html: catalog management for clothing and fabric items.

Frontend scripts:

- script.js: customer-side business logic.
- admin.js: admin dashboard operations.
- add-items.js: admin catalog operations.

Styles:

- styles.css: customer page styling.
- admin.css: admin page styling.

### 3.1.2 Backend File Organization

Server bootstrap:

- src/server.js

Express app config:

- src/app.js

Database:

- src/config/db.js

API routes:

- src/routes/index.js

Business logic:

- src/controllers/orderController.js
- src/controllers/adminController.js
- src/controllers/catalogController.js

Data models:

- src/models/Order.js
- src/models/CatalogItem.js

### 3.1.3 Design Rationale

- Centralized API helper pattern on frontend reduces duplicate fetch logic.
- Controller-based backend keeps route definitions simple.
- Mongoose schema enforces order and catalog data consistency.
- Static file serving from Express makes deployment straightforward.
- Token-based admin auth protects all privileged operations.

## 3.2 Input Design

Input design ensures all user and admin data capture is consistent, validated, and processable.

### 3.2.1 Customer Inputs

Selection inputs:

- Clothing item.
- Fabric item.
- Quantity adjustments in cart.

Form inputs at checkout:

- Full name.
- Email.
- Phone number.
- Special instructions.

Measurement inputs:

- Full shoulder
- Full sleeve
- Full chest
- Waist
- Hips
- Front chest
- Back chest
- Jacket length
- Pants waist
- Low hip
- Thigh
- Crotch
- Pant length
- Bicep
- Neck
- Cuffs

Validation approach:

- Required fields checked before submission.
- Phone pattern constrained at form level.
- Backend re-validates critical fields.

### 3.2.2 Admin Inputs

- Username and password in admin login form.
- Action buttons for status update and delete.
- File upload input for adding clothing and fabric images.
- Text input for catalog item name.

### 3.2.3 Storage Inputs

Browser storage keys used:

- tailorCart
- tailorMeasurements
- customerDetails
- orders

MongoDB collections used:

- CatalogItem
- Order

## 3.3 Output Design

Output design focuses on user feedback clarity and operational visibility.

### 3.3.1 Customer Outputs

- Dynamic cart count badge.
- Cart summary with total items and total price.
- Measurement preview block.
- Order success confirmation banner.
- Orders history cards with current status labels.
- Empty-state messages for no cart or no orders scenarios.

### 3.3.2 Admin Outputs

- Dashboard order table with date, contact, item info, measurements, and status.
- Status badges for Pending and Completed.
- Success and error message banners.
- API health test result message.
- Catalog cards with item previews and delete actions.

### 3.3.3 API Outputs

Example response from order creation:

```json
{
  "id": "<mongo-id>",
  "status": "Pending",
  "createdAt": "<timestamp>"
}
```

Example list response:

```json
[
  {
    "id": "...",
    "created_at": "...",
    "name": "...",
    "phone": "...",
    "email": "...",
    "clothing": "...",
    "fabric": "...",
    "measurements": {},
    "instructions": "...",
    "total": 0,
    "status": "Pending"
  }
]
```

## 3.4 Database Design

The database design is based on two primary collections: orders and catalog items.

### 3.4.1 Entity Overview

Entity: Order

- customer info
- item list
- measurements
- pricing
- status
- timestamps

Entity: CatalogItem

- type
- name
- image
- timestamps

### 3.4.2 Order Schema Fields

| Field        | Type         | Description                      |
| ------------ | ------------ | -------------------------------- |
| name         | String       | Customer name (required)         |
| email        | String       | Customer email                   |
| phone        | String       | Customer phone (required)        |
| instructions | String       | Optional notes                   |
| items        | Array<Item>  | Selected clothing-fabric entries |
| measurements | Mixed/Object | Body measurements                |
| clothing     | String       | Combined clothing summary        |
| fabric       | String       | Combined fabric summary          |
| total        | Number       | Total order amount               |
| status       | String enum  | Pending or Completed             |
| createdAt    | Date         | Auto-generated timestamp         |
| updatedAt    | Date         | Auto-generated timestamp         |

### 3.4.3 Catalog Schema Fields

| Field     | Type        | Description                |
| --------- | ----------- | -------------------------- |
| type      | String enum | clothing or fabric         |
| name      | String      | Item name                  |
| image     | String      | Base64 or asset path image |
| createdAt | Date        | Auto-generated timestamp   |
| updatedAt | Date        | Auto-generated timestamp   |

### 3.4.4 Data Integrity Considerations

- Required constraints on name and phone.
- Status restricted to enum values.
- Quantity and price have minimum bounds.
- Backend computes fallback total if missing.
- Catalog type is restricted to clothing or fabric.

### 3.4.5 Database Connectivity

- MongoDB URI from environment variable.
- Connection established using Mongoose.
- Graceful degraded startup when DB is unavailable.

## 3.5 System Development

This section explains implementation decisions and development process.

### 3.5.1 Development Lifecycle Followed

1. Requirement understanding
2. Frontend prototype creation
3. Cart and measurement logic implementation
4. Backend API creation
5. Database schema integration
6. Admin dashboard integration
7. Catalog storage migration to MongoDB
8. Testing and bug fixes
9. Documentation

### 3.5.2 Frontend Development Highlights

- Created responsive pages for customer flow.
- Implemented reusable Cart class in JavaScript.
- Used localStorage for persistent cart and temporary customer context.
- Added order history tracking with backend status synchronization.
- Built custom in-page confirmation modals for selection review.

### 3.5.3 Backend Development Highlights

- Built RESTful routes under /api.
- Implemented controller methods for create, list, update, and delete.
- Added object ID validation before critical operations.
- Structured JSON responses for consistent frontend handling.
- Added JWT-based admin authentication and protected endpoints.
- Added catalog persistence in MongoDB through a dedicated collection.

### 3.5.4 Admin Development Highlights

- Implemented authenticated admin login through the backend API.
- Added token-based session handling for admin actions.
- Added order table rendering and action controls.
- Added test connection feature using health endpoint.
- Added separate add-items interface for catalog updates.

### 3.5.5 Error Handling Strategy

Frontend:

- Try/catch around API requests.
- User-visible message banners.
- Empty-state fallback UI.

Backend:

- 400 for invalid input.
- 404 for missing resources.
- 500 for unexpected failures.

### 3.5.6 Security and Reliability Notes

Current phase includes practical admin authentication and protected API access.

- Admin credentials are verified on the backend instead of being hardcoded in the browser.
- Password checks use hashed credentials, and successful login returns a token for protected requests.
- Environment variables protect sensitive values such as the database URI and JWT secret.

### 3.5.7 Description of Modules

#### Module A: Customer Storefront Module

Responsibilities:

- Display hero and product-selection sections.
- Render clothing and fabric cards.
- Handle selection events and item addition.

Primary files:

- index.html
- script.js
- styles.css

#### Module B: Cart and Measurement Module

Responsibilities:

- Show selected items and totals.
- Allow quantity updates and item removal.
- Save and display measurement data.

Primary files:

- cart.html
- script.js

#### Module C: Checkout and Order Submission Module

Responsibilities:

- Display order summary.
- Capture customer contact details.
- Send normalized order payload to backend API.

Primary files:

- order.html
- script.js
- backend order creation endpoint

#### Module D: Customer Order Tracking Module

Responsibilities:

- Display local order history.
- Sync status values from backend using remote IDs.

Primary files:

- yorders.html
- script.js

#### Module E: Admin Dashboard Module

Responsibilities:

- Authenticated admin login gate.
- Load and render all backend orders.
- Mark completed, delete one, delete all.
- Handle logout and protected request headers.

Primary files:

- admin.html
- admin.js

#### Module F: Catalog Management Module

Responsibilities:

- Add new clothing and fabric entries with image upload.
- Delete catalog entries.
- Persist catalog data in MongoDB.

Primary files:

- add-items.html
- add-items.js
- catalog API endpoints

#### Module G: Backend API Module

Responsibilities:

- Route definitions.
- Request validation.
- Business logic execution.
- Database operations.

Primary files:

- src/routes/index.js
- src/controllers/orderController.js
- src/controllers/adminController.js
- src/controllers/catalogController.js
- src/models/Order.js
- src/models/CatalogItem.js

#### Module H: Database Module

Responsibilities:

- Connect application to MongoDB Atlas.
- Maintain order and catalog schema integrity.

Primary files:

- src/config/db.js
- src/models/Order.js
- src/models/CatalogItem.js

---

# CHAPTER 4 - TESTING AND IMPLEMENTATION

## 4.1 System Testing

Testing verifies whether each functional unit behaves as expected and whether integrated modules satisfy project requirements.

### 4.1.1 Testing Types Used

- Unit-level functional checks.
- API endpoint testing.
- Integration testing between frontend and backend.
- UI interaction testing.
- Negative or error path testing.

### 4.1.2 Sample Test Cases

#### Test Case 1: Add Item to Cart

- Input: Select one clothing and one fabric.
- Expected: Item appears in cart, cart count increments.
- Result: Pass.

#### Test Case 2: Checkout Without Required Name or Phone

- Input: Submit order form with missing required fields.
- Expected: Browser or backend validation blocks request.
- Result: Pass.

#### Test Case 3: Create Order API

- Input: POST /api/orders with valid payload.
- Expected: 201 response with id, status, and createdAt.
- Result: Pass.

#### Test Case 4: List Orders in Admin

- Input: Open dashboard and refresh.
- Expected: Table displays latest orders first.
- Result: Pass.

#### Test Case 5: Mark Order Completed

- Input: PATCH /api/orders/:id/complete.
- Expected: Status changes to Completed.
- Result: Pass.

#### Test Case 6: Delete Single Order

- Input: DELETE /api/orders/:id.
- Expected: Record removed and success message shown.
- Result: Pass.

#### Test Case 7: Delete All Orders

- Input: DELETE /api/orders with confirmation.
- Expected: All records removed.
- Result: Pass.

#### Test Case 8: Health Endpoint

- Input: GET /api/health.
- Expected: API is running.
- Result: Pass.

#### Test Case 9: Invalid Order ID Completion

- Input: PATCH with malformed id.
- Expected: 400 invalid order id.
- Result: Pass.

#### Test Case 10: DB Unavailable Startup

- Input: Start server with invalid URI.
- Expected: Server starts in degraded mode and DB actions fail gracefully.
- Result: Pass.

#### Test Case 11: Admin Login Success

- Input: Valid admin username and password.
- Expected: JWT token returned and dashboard opens.
- Result: Pass.

#### Test Case 12: Catalog Upload Success

- Input: Upload clothing or fabric image through add-items page.
- Expected: Item saved in MongoDB and appears in both admin and storefront catalog.
- Result: Pass.

### 4.1.3 Browser Compatibility Testing

Browsers tested:

- Google Chrome
- Microsoft Edge

Expected behavior validated:

- Responsive layout
- Form controls
- localStorage and sessionStorage behavior
- API communication
- Catalog refresh after backend updates

### 4.1.4 Performance Observations

- Local rendering is smooth for low-to-moderate order volume.
- API response is quick for normal testing data sizes.
- Base64 image storage in MongoDB works well for a project of this scale.
- Very large image files can increase database size and should be optimized later.

## 4.2 Implementation

Implementation describes practical steps to run the project end-to-end.

### 4.2.1 Deployment or Run Steps (Local)

1. Open project root in VS Code.
2. Open terminal and start the backend from the root workspace.
3. Install dependencies if needed.
4. Configure environment variable MONGODB_URI in .env.
5. Configure admin auth variables in the backend .env file.
6. Start the backend with npm start.
7. Open browser at the local app URL.
8. Test customer order flow.
9. Open admin page and perform dashboard actions.
10. Use add-items page to manage MongoDB catalog entries.

### 4.2.2 Implementation Notes

- Backend serves both API and frontend static pages.
- API base is derived from current window origin with /api suffix.
- Orders persist in MongoDB; cart and measurements persist in browser storage.
- Admin dashboard requests include backend-issued auth tokens.
- Catalog items are stored in MongoDB and loaded by both storefront and admin pages.

### 4.2.3 Operational Checklist

- Verify the API health endpoint before testing full flow.
- Ensure MongoDB Atlas IP access includes the current machine.
- Avoid exposing .env files publicly.
- Ensure the backend process on port 3001 is restarted after code changes.

### 4.2.4 Practical Implementation Challenges Faced

1. Handling mixed frontend payload shapes during API migration.
   - Solved by normalizing items in the backend controller.

2. Supporting graceful startup when DB temporarily unavailable.
   - Solved by degraded mode startup logic.

3. Keeping catalog updates synchronized across tabs.
   - Solved by a BroadcastChannel refresh mechanism.

4. Displaying verbose measurement fields in a compact admin table.
   - Solved using labels and formatted HTML rendering.

5. Moving catalog storage from browser storage to MongoDB.
   - Solved by adding a dedicated CatalogItem collection and REST endpoints.

---

# CHAPTER 5 - CONCLUSION

Urban Atelier successfully demonstrates a complete custom tailoring management workflow using modern web technologies. The project transforms a manual, error-prone process into a structured digital pipeline. Customers can place orders conveniently with detailed measurements, while administrators gain better operational control through a centralized dashboard.

Technically, the project achieves:

- Frontend-backend separation with clear module boundaries.
- API-driven data flow and structured error handling.
- Persistent and query-friendly order and catalog storage in MongoDB.
- Maintainable architecture suitable for future production hardening.
- Secure admin authentication using backend verification and JWT tokens.

From an academic standpoint, the project validates practical understanding of full-stack development, database schema design, and deployment-oriented coding practices.

## 5.1 Future Enhancements

Recommended next improvements:

- Payment gateway integration through UPI, cards, or wallets.
- Delivery and pickup slot scheduling.
- Real-time notifications via SMS, email, or WhatsApp.
- Product image optimization and object storage integration.
- Analytics dashboard for order trends and revenue insights.
- Multi-language UI support.
- Advanced search, filter, and export in the admin panel.
- Role-based access control for multiple admin users.

---

# BIBLIOGRAPHY

1. Express.js Documentation, https://expressjs.com/
2. Mongoose Documentation, https://mongoosejs.com/docs/
3. MongoDB Documentation, https://www.mongodb.com/docs/
4. Node.js Documentation, https://nodejs.org/docs/
5. MDN Web Docs for HTML, CSS, and JavaScript, https://developer.mozilla.org/
6. REST API design references and backend implementation notes.
7. Course notes and faculty guidance on software engineering and web systems.

---

# APPENDICES

## Appendix A - Data Flow Diagram (DFD)

### A.1 Context-Level DFD (Level 0)

External entities:

- Customer
- Admin
- MongoDB Database

System process:

- Urban Atelier Web System

Data flow summary:

- Customer sends order details and receives status view.
- Admin sends management actions and receives dashboard data.
- System writes and reads orders and catalog items to and from the database.

### A.2 Level 1 DFD (Suggested Process Breakdown)

P1: Customer Selection and Cart Process
P2: Measurement and Checkout Process
P3: Order API Processing
P4: Admin Order Management
P5: Catalog Management Process
P6: Status Synchronization Process

(Insert DFD diagrams here as screenshots or images in the final document.)

## Appendix B - Database Table Structure

### B.1 orders Collection

| Column or Field | Type     | Nullable | Key | Default | Remarks                     |
| --------------- | -------- | -------- | --- | ------- | --------------------------- |
| \_id            | ObjectId | No       | PK  | Auto    | Unique order ID             |
| name            | String   | No       | -   | -       | Customer name               |
| email           | String   | Yes      | -   | Empty   | Email address               |
| phone           | String   | No       | -   | -       | Contact number              |
| instructions    | String   | Yes      | -   | Empty   | Customer notes              |
| items           | Array    | Yes      | -   | Empty   | Clothing and fabric entries |
| measurements    | Object   | Yes      | -   | Empty   | Body measurements           |
| clothing        | String   | Yes      | -   | Empty   | Combined clothing summary   |
| fabric          | String   | Yes      | -   | Empty   | Combined fabric summary     |
| total           | Number   | Yes      | -   | 0       | Order amount                |
| status          | String   | Yes      | -   | Pending | Pending or Completed        |
| createdAt       | Date     | No       | -   | Auto    | Creation timestamp          |
| updatedAt       | Date     | No       | -   | Auto    | Last update timestamp       |

### B.2 CatalogItem Collection

| Column or Field | Type     | Nullable | Key     | Default | Remarks                    |
| --------------- | -------- | -------- | ------- | ------- | -------------------------- |
| \_id            | ObjectId | No       | PK      | Auto    | Unique catalog item ID     |
| type            | String   | No       | Indexed | -       | clothing or fabric         |
| name            | String   | Yes      | -       | Empty   | Display name               |
| image           | String   | No       | -       | -       | Asset path or base64 image |
| createdAt       | Date     | No       | -       | Auto    | Creation timestamp         |
| updatedAt       | Date     | No       | -       | Auto    | Last update timestamp      |

## Appendix C - API Reference

### C.1 Authentication

- `POST /api/admin/login` -> returns JWT token for admin access.

### C.2 Order APIs

- `POST /api/orders` -> create a new order.
- `GET /api/orders` -> list all orders.
- `GET /api/orders/statuses?ids=id1,id2` -> fetch selected order statuses.
- `PATCH /api/orders/:id/complete` -> mark an order as completed.
- `DELETE /api/orders/:id` -> delete one order.
- `DELETE /api/orders` -> delete all orders.

### C.3 Catalog APIs

- `GET /api/catalog?type=clothing` -> list clothing items.
- `GET /api/catalog?type=fabric` -> list fabric items.
- `POST /api/catalog` -> create a catalog item.
- `DELETE /api/catalog/:id` -> delete a catalog item.

## Appendix D - Code Snippets

### D.1 Admin Login Flow

```javascript
async function handleLogin(event) {
  event.preventDefault();
  const username = document.getElementById("username").value.trim();
  const password = document.getElementById("password").value.trim();

  const payload = await apiRequest("/admin/login", {
    method: "POST",
    body: JSON.stringify({ username, password }),
  });

  sessionStorage.setItem("adminLoggedIn", "true");
  sessionStorage.setItem("adminToken", payload.token);
}
```

### D.2 Catalog Item Creation

```javascript
exports.createCatalogItem = async (req, res) => {
  const createdItem = await CatalogItem.create({
    type,
    name,
    image,
  });

  return res.status(201).json({
    id: String(createdItem._id),
    type: createdItem.type,
    name: createdItem.name || "",
    image: createdItem.image,
  });
};
```

### D.3 Storefront Catalog Loading

```javascript
async function loadCatalogItemsFromApi() {
  const [clothingItems, fabricItems] = await Promise.all([
    apiRequest("/catalog?type=clothing"),
    apiRequest("/catalog?type=fabric"),
  ]);
}
```

## Appendix E - Screenshots

Insert screenshots here in the final Word/PDF version.
Suggested screenshots:

1. Home page
2. Clothing selection section
3. Fabric selection section
4. Confirmation modal
5. Cart page
6. Measurement form
7. Order submission page
8. Customer orders page
9. Admin login page
10. Admin dashboard
11. Add-items page
12. Catalog upload result
13. Catalog deletion result
14. MongoDB collection view
15. API test / health endpoint

---

# ONE-SCREEN MENTAL MODEL

If you forget the details, remember this:

- `script.js` runs almost all customer-side behavior.
- `admin.js` runs order-management behavior for admin.
- `add-items.js` runs catalog-management behavior for admin.
- `localStorage` handles cart, measurements, and order history.
- MongoDB handles real order persistence, admin-auth data, and catalog entries.
- Express + JWT handle protected admin operations and API routing.

That is the current full project model.
